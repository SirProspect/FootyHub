    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 30) {
                ...
                .alert("Load Real Data", isPresented: $showAPIAlert) {
                    Button("Yes, Load It") {
                        dataHolder.fetchFromAPI(context)
                    }
                    Button("Cancel", role: .cancel) {}
                } message: {
                    Text("This will fetch real teams and players from TheSportsDB and replace existing data. Only do this once!")
                }
